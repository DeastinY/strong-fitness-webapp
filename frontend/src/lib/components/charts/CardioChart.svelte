			...
			options: {
				...
				plugins: {
					...
					tooltip: {
						callbacks: {
							label: ctx => {
								if (ctx.datasetIndex === 0) return `Distance arc: ${(ctx.parsed.y ?? 0).toFixed(2)} km`;
								return `Time arc: ${ctx.parsed.y ?? 0} min`;
							}
						}
					}
				},
				scales: {
					...
					y: {
						...
						title: { display: true, text: 'Distance (km)', color: labelColor, font: { size: 10 } }
					},
					y1: {
						position: 'right',
						beginAtZero: false,
						...
						ticks: { color: tickColor, maxTicksLimit: 5 },
						title: { display: true, text: 'Time (min)', color: labelColor, font: { size: 10 } }
					}
				}
			}
		});
...
<Card class="p-4 sm:p-6">
	<h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-3">Cardio Side Quest</h3>
	<div class="h-48 sm:h-56">
		{#if data.length > 0}
			<canvas bind:this={canvas}></canvas>
		{:else}
			<div class="h-full flex items-center justify-center text-sm text-gray-500 dark:text-gray-400">
				No cardio side-quest data yet
			</div>
		{/if}
	</div>
	{#if data.length > 0}
		...
		<div class="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700/50 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
			<span>Runs logged: <span class="font-semibold text-gray-900 dark:text-white">{data.length}</span></span>
			<span>Avg distance arc: <span class="font-semibold text-gray-900 dark:text-white">{avgKm.toFixed(2)} km</span></span>
			<span>Avg time arc: <span class="font-semibold text-gray-900 dark:text-white">{avgMin} min</span></span>
		</div>
	{/if}
</Card>
